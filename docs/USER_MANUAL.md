...
| **GMX** | 永续合约 / GLP 收益 | 注册表协议（Arbitrum Sepolia） | Medium |
...
#### 借贷（Lending）— Aave V3 / Compound V3

将 ETH 存入借贷池，借款人支付利息，存款人赚取利息差。

| 特征 | 说明 |
|------|------|
| 收益来源 | 借款人支付的利息 |
| 凭证类型 | ERC-20（aWETH / Comet 份额），余额随利息自动增长（rebasing） |
| 风险点 | 借款人违约（有清算机制保障）、智能合约漏洞 |
| 年化收益 | 约 2-4%（视市场供需） |

#### ETH 质押（Liquid Staking）— Lido

将 ETH 委托给验证节点参与以太坊 PoS 共识，获得区块奖励。

| 特征 | 说明 |
|------|------|
| 收益来源 | 以太坊 PoS 区块奖励 |
| 凭证类型 | ERC-20（stETH），余额每天增长反映质押奖励（rebasing） |
| 风险点 | 验证节点被 slash（罚没）、提款排队时间 |
| 年化收益 | 约 3-4%（较稳定） |

#### 永续合约收益（Perp Yield）— GMX

通过购买 GLP（GMX Liquidity Provider Token）为 GMX 永续合约交易所提供流动性，获取交易手续费和资金费率分成。

| 特征 | 说明 |
|------|------|
| 收益来源 | GMX 永续合约交易者支付的手续费 + 资金费率 |
| 凭证类型 | ERC-20（GLP），可随时赎回为底层资产 |
| 风险点 | 交易者大量盈利时 GLP 持有者承担对手方亏损、底层资产价格波动 |
| 年化收益 | 约 5-15%（与交易量和市场波动正相关） |
| 部署链 | Arbitrum（主网 / Arbitrum Sepolia 测试网） |

> **当前状态**：GMX 已注册在后端协议注册表中（Arbitrum Sepolia，ChainID 421614），AI 可推荐 GLP Vault 策略。Adapter 合约未部署时走 Solver 模式。

#### DEX 流动性提供（LP）— Uniswap V3

将资产放入 DEX 交易池，交易者每次兑换支付手续费，LP 按份额分成。

| 特征 | 说明 |
|------|------|
| 收益来源 | 交易者支付的手续费（与交易量正相关） |
| 凭证类型 | ERC-721 NFT（Uni V3 特有，有价格区间参数） |
| 风险点 | **无常损失（Impermanent Loss）**— 资产价格偏离导致的隐性亏损 |
| 年化收益 | 约 5-20%（波动大，与交易活跃度相关） |

### 统一适配器架构

DeFi Pilot 通过 **Adapter 适配器模式** 将三种完全不同的协议交互方式统一封装。Vault 合约不感知底层协议类型，只关心标准接口：

```
统一接口（所有 Adapter 必须实现）：
┌──────────────────────────────────────────────┐
│  aWETH() → address         // 返回凭证地址   │
│  depositETH(address) payable // 存入 ETH     │
│  withdrawETH(uint256, address) // 取回 ETH   │
└──────────────────────────────────────────────┘
     │           │           │           │
┌────┴───┐ ┌────┴────┐ ┌────┴───┐ ┌─────┴─────┐
│ Aave   │ │Compound │ │ Lido   │ │ Uniswap   │
│Adapter │ │Adapter  │ │Adapter │ │ V3Adapter │
│ERC-20  │ │ERC-20   │ │ERC-20  │ │ERC-20*    │
│aWETH   │ │Comet    │ │stETH   │ │dpUNI3     │
└────────┘ └─────────┘ └────────┘ └───────────┘
                                   * NFT 被 Adapter
                                     内部封装为 ERC-20
```

Uniswap V3 的 NFT LP 头寸由 Adapter 内部持有，对外通过 ERC-20 份额凭证（dpUNI3）与 Vault 交互。这意味着：

- **新增协议**：只需部署 Adapter 合约 + 后端注册，Vault 和前端**零改动**
- **用户无感**：无论底层是 rebasing token、Comet 份额还是 NFT，用户看到的都是统一的「一键存入 / 赎回」体验
...
| 区域 | 功能 |
|------|------|
| **顶部导航** | Logo、导航入口、**语言切换（中/英）**、**主题切换（亮色/暗色）**、多链状态圆点、**连接钱包** |
| **左侧聊天** | 与 AI 对话，描述投资意图；收到 **策略推荐卡片**；底部显示实时链监控状态 |
...
点击顶部导航栏右侧的 **☀️ / 🌙** 按钮，可在**暗色模式**（默认）和**亮色模式**之间切换。

- 选择会自动保存到浏览器，下次打开无需重新设置。
- 亮色模式下背景网格和粒子效果自动隐藏，渐变色调调整为更柔和的浅色系。

---

## 5.6 多链监控状态

顶部导航栏中的彩色圆点和 ChatPanel 底部的状态文案反映**实时多链 RPC 健康状态**：

- 系统每 30 秒向后端 `/api/health/chains` 查询所有链的连通性。
- **当前监控 8 条链**：Ethereum、Arbitrum、Optimism、Base、Polygon、Avalanche（主网）+ Sepolia、Arbitrum Sepolia（测试网）。
- 绿色圆点 = 该链 RPC 正常连通；红色圆点 = 连接异常。
- ChatPanel 底部显示格式如："在线 — 监控 8 条链（8 healthy）"。
...
| `docs/DEPLOYMENT.md` | 合约/后端/前端部署指南 |
| `README.md` | 项目总览 |
